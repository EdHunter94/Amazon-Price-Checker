...
class MWS:
    ...
    def make_request(self, request_data):
        ...
        request_description = '&'.join(['%s=%s' % (k, ur.quote(data[k], safe='-_.~').encode('utf-8')) for k in sorted(data)])
        ...
        #print request
        try:
            xml = ur.urlopen(urllib.Request(request, headers={'User-Agent': self.user_agent})).read()
    ...
    def get_timestamp(self):
        ...
        print("yes")
        timestamp_html = ur.urlopen("https://mws.amazonservices.com/").read()
        return re.search(b'timestamp="(.*?)"', timestamp_html).groups()[0]
